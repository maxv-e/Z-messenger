...
async def init_db():
    global db_pool
    db_pool = await aiosqlite.connect(DB_PATH, timeout=SQLITE_TIMEOUT)
    ...
    await db_pool.execute('''CREATE TABLE IF NOT EXISTS user_keys (
        user_id INTEGER PRIMARY KEY,
        public_key_b64 TEXT NOT NULL,
        updated_at INTEGER
    )''')

    await db_pool.execute('''CREATE TABLE IF NOT EXISTS groups (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
    ...
    await db_pool.execute('''CREATE TABLE IF NOT EXISTS group_members (
        group_id INTEGER, user_id INTEGER, joined_at INTEGER,
        PRIMARY KEY(group_id, user_id)
    )''')

    await db_pool.execute('''CREATE TABLE IF NOT EXISTS channels (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
        type TEXT CHECK(type IN ('private','public')) NOT NULL,
        owner_id INTEGER NOT NULL,
    ...
    await db_pool.execute('''CREATE TABLE IF NOT EXISTS channel_subscribers (
        channel_id INTEGER, user_id INTEGER, subscribed_at INTEGER,
        PRIMARY KEY(channel_id, user_id)
    )''')

    await db_pool.execute('''CREATE TABLE IF NOT EXISTS messages (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    await db_pool.execute('''CREATE TABLE IF NOT EXISTS pending_messages (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    await db_pool.execute('''CREATE TABLE IF NOT EXISTS sessions (
    ...
    await db_pool.execute('''CREATE TABLE IF NOT EXISTS failed_logins (
        ip TEXT NOT NULL,
        attempt_time INTEGER NOT NULL
    )''')

    await db_pool.execute("CREATE INDEX IF NOT EXISTS idx_messages_recipient ON messages(recipient_type, recipient_id, timestamp)")
    await db_pool.execute("CREATE INDEX IF NOT EXISTS idx_messages_sender ON messages(sender_id, timestamp)")
    await db_pool.execute("CREATE INDEX IF NOT EXISTS idx_pending_recipient ON pending_messages(recipient_user_id, delivered)")
    await db_pool.execute("CREATE INDEX IF NOT EXISTS idx_sessions_user ON sessions(user_id)")
    await db_pool.execute("CREATE INDEX IF NOT EXISTS idx_messages_status ON messages(status, timestamp)")
    await db_pool.execute("CREATE INDEX IF NOT EXISTS idx_failed_logins_ip_time ON failed_logins(ip, attempt_time)")

    await db_pool.commit()
    logger.info("Database initialized with WAL and indexes")
...
async def db_lastrowid() -> int:
    cursor = await db_pool.execute("SELECT last_insert_rowid()")
...
async def check_rate_limit(ip: str, user_id: Optional[int] = None) -> bool:
    ...
    async with rate_limit_lock:
        ...
        if user_id is not None:
            user_times = rate_limit_user[user_id]
...
async def get_user_public_key(user_id: int) -> Optional[bytes]:
    row = await db_execute("SELECT public_key_b64 FROM user_keys WHERE user_id=?", (user_id,), fetchone=True)
    if row:
        return base64.b64decode(row['public_key_b64'])
    return None

async def store_user_public_key(user_id: int, public_key_b64: str):
    await db_execute(
        "INSERT OR REPLACE INTO user_keys (user_id, public_key_b64, updated_at) VALUES (?,?,?)",
...
async def recv_encrypted(reader: asyncio.StreamReader, cipher: ChaCha20Poly1305, expected_seq: int) -> Tuple[Optional[dict], int, bool]:
    """Возвращает (data, next_seq, connection_closed)"""
    try:
        ...
        if msg_len > MAX_MESSAGE_SIZE:
            ...
            return None, expected_seq, False
        ...
        if seq > expected_seq + MAX_SEQ_WINDOW:
            ...
            return None, expected_seq, False
        return payload.get('data'), seq + 1, False
    except asyncio.TimeoutError:
        return None, expected_seq, False
    except asyncio.IncompleteReadError:
        logger.debug("Connection closed by peer")
        return None, expected_seq, True
        ...
        async with ctx.send_lock:
            for row in pending:
                ...
                try:
                    ...
                    asyncio.create_task(wait_for_pending_ack(future, pending_id, ctx, user_id))
                except Exception as e:
                    logger.error(f"Failed to send pending message {pending_id}: {e}")
        offset += BATCH_OFFLINE_SIZE
        await asyncio.sleep(0.1)

async def wait_for_pending_ack(future: asyncio.Future, pending_id: int, ctx: ConnectionContext, recipient_id: int):
    try:
...
async def wait_for_message_ack(future: asyncio.Future, original_msg_id: int, ctx: ConnectionContext, sender_id: int):
    try:
        await asyncio.wait_for(future, timeout=ACK_TIMEOUT)
                    ...
                    for ctx in active_connections[user_id]:
                        ...
                        async with ctx.send_lock:
                            for pending_id, data in msgs:
                                try:
                                    await send_encrypted(ctx.writer, ctx.cipher, data['msg'], ctx.send_seq)
                                    future = asyncio.get_event_loop().create_future()
                                    ctx.pending_acks[ctx.send_seq] = (future, pending_id, data.get('original_msg_id'))
                                    ctx.send_seq += 1
                                    await db_execute("UPDATE pending_messages SET delivered=1 WHERE id=?", (pending_id,))
                                    asyncio.create_task(wait_for_pending_ack(future, pending_id, ctx, user_id))
                                except Exception as e:
...
async def broadcast_message(
    ...
    if recipient_type == 'personal':
        ...
        if contexts:
            for ctx in contexts:
                if not ctx.writer.is_closing():
                    await send_to_connection(ctx, base_msg, message_id, sender_id)
...
async def send_to_connection(ctx: ConnectionContext, msg_dict: dict, original_msg_id: int, sender_id: int):
    async with ctx.send_lock:
        seq = ctx.send_seq
        await send_encrypted(ctx.writer, ctx.cipher, msg_dict, seq)
        future = asyncio.get_event_loop().create_future()
        ctx.pending_acks[seq] = (future, None, original_msg_id)
        ctx.send_seq += 1
        asyncio.create_task(wait_for_message_ack(future, original_msg_id, ctx, sender_id))

async def process_command(user_id: int, cmd: dict, ctx: ConnectionContext) -> Tuple[Optional[dict], int]:
    action = cmd.get('action')
    if action == 'send_message':
        if not await check_rate_limit(ctx.ip, user_id):
            return {'error': 'rate_limit_exceeded'}, ctx.send_seq
        rec_type = cmd.get('recipient_type')
        rec_id = cmd.get('recipient_id')
        content = cmd.get('content', '')
        if not rec_type or not rec_id or not content:
            return {'error': 'invalid_params'}, ctx.send_seq
        if not await can_send_to_recipient(user_id, rec_type, rec_id):
            return {'error': 'access_denied'}, ctx.send_seq
        timestamp = int(time.time())
        if rec_type == 'personal':
            pub_key = await get_user_public_key(rec_id)
            if not pub_key:
                return {'error': 'recipient_no_e2ee_key'}, ctx.send_seq
        ...
        return {'status': 'ok', 'message_id': msg_id}, ctx.send_seq
        ...
        if not user:
            return {'error': 'user_not_found'}, ctx.send_seq
        return {'action': 'profile', 'data': dict(user)}, ctx.send_seq
        ...
        if 'username' in cmd:
            ...
            if new_name:
                ...
                if exist:
                    return {'error': 'username_taken'}, ctx.send_seq
        ...
        if b64_data:
            try:
                ...
                if len(img_bytes) > 2*1024*1024:
                    return {'error': 'avatar_too_large'}, ctx.send_seq
                ...
                return {'error': 'invalid_image'}, ctx.send_seq
        return {'error': 'no_data'}, ctx.send_seq
        ...
        if group_type not in ('private', 'public'):
            group_type = 'private'
        if not name:
            return {'error': 'invalid_name'}, ctx.send_seq
        ts = int(time.time())
        ...
        return {'status': 'ok', 'group_id': group_id}, ctx.send_seq
        ...
        return {'status': 'ok', 'channel_id': channel_id}, ctx.send_seq
        ...
        return {'action': 'groups_list', 'groups': [dict(g) for g in groups]}, ctx.send_seq
        ...
        ''', (user_id,), fetchall=True)
        return {'action': 'channels_list', 'channels': [dict(c) for c in channels]}, ctx.send_seq

    elif action == 'join_group':
        group_id = cmd.get('group_id')
        if not group_id:
            return {'error': 'no_group'}, ctx.send_seq
        group = await db_execute("SELECT type FROM groups WHERE id=?", (group_id,), fetchone=True)
        if not group:
            return {'error': 'not_found'}, ctx.send_seq
        if group['type'] == 'private':
            owner = await db_execute("SELECT owner_id FROM groups WHERE id=?", (group_id,), fetchone=True)
            if owner['owner_id'] != user_id:
                return {'error': 'private_group_cannot_join'}, ctx.send_seq
        ts = int(time.time())
        try:
            await db_execute("INSERT INTO group_members (group_id, user_id, joined_at) VALUES (?,?,?)", (group_id, user_id, ts))
        except aiosqlite.IntegrityError:
            return {'error': 'already_member'}, ctx.send_seq
        return {'status': 'ok'}, ctx.send_seq

    elif action == 'subscribe_channel':
        channel_id = cmd.get('channel_id')
        if not channel_id:
            return {'error': 'no_channel'}, ctx.send_seq
        channel = await db_execute("SELECT type FROM channels WHERE id=?", (channel_id,), fetchone=True)
        if not channel:
            return {'error': 'not_found'}, ctx.send_seq
        ...
        if rec_type == 'personal':
            ...
            return {'error': 'bad_type'}, ctx.send_seq
        return {'action': 'history', 'messages': [dict(m) for m in msgs][::-1]}, ctx.send_seq
        ...
        if result:
            ...
            return {'status': 'ok', 'user_id': user_id_from_token, 'device_id': device_id}, ctx.send_seq
        else:
            return {'error': 'invalid_token'}, ctx.send_seq
        ...
        return {'action': 'sync_unread', 'messages': [dict(m) for m in msgs]}, ctx.send_seq
        ...
        if public_key_b64:
            ...
            return {'status': 'ok'}, ctx.send_seq
        return {'error': 'no_key'}, ctx.send_seq

    elif action == 'get_e2ee_key':
        target_id = cmd.get('user_id')
        if not target_id:
            return {'error': 'no_target'}, ctx.send_seq
        pub_key = await get_user_public_key(target_id)
        if pub_key:
            return {'public_key_b64': base64.b64encode(pub_key).decode()}, ctx.send_seq
        else:
            return {'error': 'no_key'}, ctx.send_seq

    elif action == 'ping':
        return {'pong': time.time()}, ctx.send_seq
        ...
        return {'error': 'unknown_action'}, ctx.send_seq
...
async def handle_client(reader: asyncio.StreamReader, writer: asyncio.StreamWriter, server_private_key: X25519PrivateKey):
    ...
    async def reader_loop():
        ...
        try:
            while True:
                try:
                    cmd, new_recv_seq, closed = await recv_encrypted(reader, cipher, ctx.recv_seq)
                    if closed:
                ...
                if authenticated_user is None:
                    if cmd.get('action') == 'auth':
                        ...
                        if auth_type == 'register':
                            ...
                            if existing:
                                await send_encrypted(writer, cipher, {'error': 'login_exists'}, ctx.send_seq)
                                ctx.send_seq += 1
                                continue
                            ...
                            await send_encrypted(writer, cipher, {'status': 'ok', 'token': token, 'user_id': user_id}, ctx.send_seq)
                            ctx.send_seq += 1
                            ...
                            if not user:
                                ...
                                await send_encrypted(writer, cipher, {'error': 'invalid_credentials'}, ctx.send_seq)
                                ctx.send_seq += 1
                                continue
                            stored = user['password_hash']
                            if ':' not in stored:
                                await send_encrypted(writer, cipher, {'error': 'invalid_hash'}, ctx.send_seq)
                                ctx.send_seq += 1
                                continue
                            ...
                            if verify_password(password, pwd_hash, salt):
                                ...
                                await send_encrypted(writer, cipher, {'status': 'ok', 'token': token, 'user_id': user['id']}, ctx.send_seq)
                                ctx.send_seq += 1
                            ...
                            await send_encrypted(writer, cipher, {'error': 'auth_required'}, ctx.send_seq)
                            ctx.send_seq += 1
                        ...
                        if result:
                            ...
                            async with active_connections_lock:
                                active_connections.setdefault(user_id, []).append(ctx)
                            await async_deliver_pending_messages(user_id, ctx)
                            await send_encrypted(writer, cipher, {'status': 'ok', 'user_id': user_id}, ctx.send_seq)
                            ctx.send_seq += 1
                            ...
                            await send_encrypted(writer, cipher, {'error': 'invalid_token'}, ctx.send_seq)
                            ctx.send_seq += 1
                    else:
                        await send_encrypted(writer, cipher, {'error': 'authenticate_first'}, ctx.send_seq)
                        ctx.send_seq += 1
                    continue

                try:
                    resp, new_send_seq = await process_command(authenticated_user, cmd, ctx)
                    if resp:
                        async with ctx.send_lock:
                    ...
                    try:
                        await send_encrypted(writer, cipher, {'error': 'internal_error'}, ctx.send_seq)
                        ctx.send_seq += 1
                    except:
                        pass
        except Exception as e:
    ...
    try:
        ...
        for future, _, _ in ctx.pending_acks.values():
            if not future.done():
                future.cancel()
...
class ZSecureClient:
    ...
    async def authenticate(self, mode, login, password, username=""):
        if mode == 'login':
            key_file = f"e2ee_key_{self.user_id if self.user_id else '?'}.bin"
            key_file = f"e2ee_key_{login}.bin"  # временно по логину
            if not os.path.exists(key_file):
                print(f"E2EE key file for {login} not found. Please register first or restore key.")
                return False
        ...
        if resp and resp.get('status') == 'ok':
            ...
            with open(key_file, "wb") as f:
                f.write(self.e2ee_private_key.private_bytes_raw())
            return True
        else:
    ...
    async def restore_session(self):
        ...
        if resp and resp.get('status') == 'ok':
            ...
            if os.path.exists(key_file):
                ...
                with open(key_file, "wb") as f:
                    f.write(self.e2ee_private_key.private_bytes_raw())
                await self.send_command({'action': 'set_e2ee_key', 'public_key_b64': self.e2ee_public_key_b64}, wait_response=False)
            return True
        else:
    ...
    async def receive_loop(self):
        while self.running:
            try:
                data, new_seq, closed = await recv_encrypted(self.reader, self.cipher, self.recv_seq)
                if closed:
                ...
                if self.pending_responses:
                    req_seq = min(self.pending_responses.keys())
                    future = self.pending_responses.pop(req_seq)
                    if not future.done():
